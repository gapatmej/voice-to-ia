...
export async function getExpensesDetail() {
  try {
    const results = await db.executeSql(`SELECT * FROM ${TABLE_EXPENSES_DETAIL};`);
    let expensesDetail = [];
...
export async function updateExpenseDetail(id, date, description, expenseType, total) {
  try {
    await db.transaction(tx => {
      tx.executeSql(
...
export async function deleteExpenseDetail(id) {
  try {
    await db.transaction(tx => {
      tx.executeSql(
        `DELETE FROM ${TABLE_EXPENSES_DETAIL} WHERE id = ?;`,
        [id]
      );
    });
    return { success: true };
